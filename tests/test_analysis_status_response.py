...
from pythx.core.response.models import AnalysisStatusResponse
...
VALID_SUBMISSION_RESPONSE = AnalysisStatusResponse(
    analysis=Analysis(
        uuid=UUID_1,
...
def test_analysis_list_from_valid_json():
    resp = AnalysisStatusResponse.from_json(json.dumps(VALID_ANALYSIS_DICT))
...
def test_analysis_list_from_valid_dict():
    resp = AnalysisStatusResponse.from_dict(VALID_ANALYSIS_DICT)
    assert_analysis_data(VALID_ANALYSIS_DICT, resp.analysis)
